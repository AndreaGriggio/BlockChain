...
int sendMessage(int fd, const Message *message_ptr)
{
    ...
    if (r != 0)
        return r; // errore se sendAll non riesce a inviare l'header
...
int receiveMessage(int fd, Message *message_ptr)
{
    ...
    {
    case MSG_NEW_TX:
        max_payload_size = MAX_TX_SIZE;
        break;
    case MSG_BLOCK_MINED:
        max_payload_size = MAX_BLOCK_TXS_BUF;
        break;
    default:
        return INVALID_PARAMS;
    }

    if (message_ptr->payload_size > max_payload_size)
    {
        return BUFFER_TOO_SMALL;
    }
