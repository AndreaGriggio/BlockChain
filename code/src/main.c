...
#include "communication/message.h" // Message, MSG_NEW_TX, sendMessage (protocolSocket.h/childProcess.h)
#include "repl.h"
#include "broker.h"
...
int main(int argc, char *argv[])
{
    ...
    */
    for (int m = 0; m < cfg.num_miners; m++)
    {
        ...
        {
            char p[64];
            snprintf(p, sizeof p, "%s%d_%d", MINER_NODE_FIFO, m, n);
            unlink(p);
            snprintf(p, sizeof p, "%s%d_%d", NODE_MINER_FIFO, n, m);
            unlink(p);
        }
    }
    ...
    {
        char p[64];
        snprintf(p, sizeof p, "%s%d", NODE_BROKER_FIFO, i);
        unlink(p);
        snprintf(p, sizeof p, "%s%d", BROKER_NODE_FIFO, i);
        unlink(p);
    }
    sem_unlink(BROKER_SEM_NAME);
    return 0;
